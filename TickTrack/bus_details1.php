...
$seats=$_POST["Seats"];
$Bus_id;
$query = "SELECT * FROM bus where Origin='{$origin}' and Destination='{$destination}' and Seats>= '{$seats}' and Date='{$dateoj}'" ;
...
while($row = mysqli_fetch_array($select_vehicle_query)){
	...
	echo "<td>";
//	echo "$req";
	echo '<a href="payment1.php?Seats_no='.$req.'& Bus_id='.$Bus_id.'& Total_fare='.$Total_fare.'& Date='.$dateoj.'">Book Now</a>';
	echo "</td>";
	echo "</tr>";
}
echo "</table>";
